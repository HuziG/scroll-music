...
// 邮箱注册
export const emailRegister = ({ email, password }: { email: string; password: string }) => {
...
// 邮箱找回密码
export const getPasswordByEmail = ({ email }: { email: string }) => {
  return BaaS.auth.requestPasswordReset({ email });
};

// 发送手机验证码
export const sendSmsCode = ({ telephone }: { telephone: string }) => {
...
// 短信一键登录
export const loginWithSmsVerificationCode = (telephone: string, code: string) =>
  BaaS.auth.loginWithSmsVerificationCode(telephone, String(code));

// 手机登录
export const telephoneLogin = ({ telephone, password }: { telephone: string; password: string }) =>
